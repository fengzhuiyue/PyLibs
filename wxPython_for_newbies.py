...
class main_window(wxFrame):
	def __init__(self, parent, id, title):
		...
		# ------------------------------------------------------------------------------------
		tID = wxNewId()
		self.tree = wxTreeCtrl (splitter, tID, style=wxTR_HAS_BUTTONS |wxTR_EDIT_LABELS |wxTR_HAS_VARIABLE_ROW_HEIGHT)
	...
	def project_save(self):
		try:
			...
			dlg_m = wxMessageDialog (self, 'There was an error saving the project file.','Error!', wxOK)
			dlg_m.ShowModal()
			dlg_m.Destroy()
	# ----------------------------------------------------------------------------------------
	...
	def OnProjectOpen(self, event):
		...
		dlg=wxMessageDialog(self, 'The project has been changed.  Save?', 'wxProject', wxYES_NO | wxCANCEL)
	...
	def OnProjectNew(self, event):
		...
		dlg.Destroy()
		if open_it:
		dlg = wxTextEntryDialog (self, "Name for new project:", "New Project", "New project", wxOK | wxCANCEL)
		if dlg.ShowModal() == wxID_OK:
			newproj = dlg.GetValue()
			dlg.Destroy()
			dlg = wxFileDialog (self, "Place to store new project", ".", "", "*.wxp",wxSAVE)
			if dlg.ShowModal() == wxID_OK:
				try:
					...
					dlg_m = wxMessageDialog (self, 'There was an error saving the new project file.', 'Error!', wxOK)
					dlg_m.ShowModal()
					dlg_m.Destroy()
		dlg.Destroy()
	...
	def OnTreeItemActivated(self, event):
		...
		if self.editor.IsModified():
			dlg=wxMessageDialog(self, 'The edited file has changed.  Save it?', 'wxProject', wxYES_NO | wxCANCEL)
			result = dlg.ShowModal()
			if result == wxID_YES:
				self.editor.SaveFile (self.tree.GetItemText (self.activeitem))
			if result == wxID_CANCEL:
